...
通俗来说，就是**“把请求当作任务来处理，先给个回执，稍后再凭回执取结果”**，而不是**“当场立即算出结果返回”**。

这完全是为了应对**大模型推理耗时长、易超时**的场景而设计的。
...
**学习率（Learning Rate, LR）**，在深度学习中通常用符号 **η** 或 **LR** 表示，它是一个**步长（Step Size）**。

在训练中，模型通过计算“损失（Loss）”来发现自己的预测有多离谱，然后计算梯度（Gradient，即下降的方向）。

- **梯度**：告诉你应该往**哪个方向**走（上坡还是下坡）。
    
- **学习率（2e-6）**：告诉你朝着那个方向**走多大的步子**。
