...
def train(cfg: TrainPipelineConfig, accelerator: Callable = None):
    ...
    # ==================== 新增：TensorBoard 初始化 ====================
    # 仅主进程创建 writer，避免多卡同时写入冲突
    if accelerator and not accelerator.is_main_process:
        writer = None
    else:
        log_dir = '/home/ma-user/summary'   # ModelArts 可视化默认读取路径
        os.makedirs(log_dir, exist_ok=True)
        writer = SummaryWriter(log_dir=log_dir)
        logging.info(colored(f"TensorBoard logs will be written to: {log_dir}", "green", attrs=["bold"]))
    # ================================================================
    ...
    for _ in range(step, cfg.steps):
        ...
        if is_log_step:
            ...
            if writer is not None:
                writer.add_scalar('Loss/train', train_tracker.loss, step)
                writer.add_scalar('LR/train', train_tracker.lr, step)
                writer.add_scalar('GradNorm/train', train_tracker.grad_norm, step)
                # 可以按需添加更多指标
            # ================================================================
